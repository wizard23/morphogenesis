...
export function timingRows(results) {
  const header = ["scenario", "P", "S", "p50 ms", "p95 ms", "p99 ms", "max ms", "spread", "constr/it", "coll/it", "bin", "ovf", "disp px", "hwm KB", "checksum"];
  const rows = results.map((r) => [
    r.id, r.alive.particles, r.alive.springs,
    fmtMs(r.p50), fmtMs(r.p95), fmtMs(r.p99), fmtMs(r.max), `${(r.spreadP50 * 100).toFixed(1)}%`,
